...
const { authMiddleware, generateToken, JWT_SECRET } = require(path.join(__dirname, 'middleware/auth.middleware.cjs'));
const { validateLogin, validateRegister, validateProperty } = require(path.join(__dirname, 'middleware/validate.middleware.cjs'));
...
async function initializeServer() {
  try {
    ...
    logger.error('Failed to initialize DataEngine:', e.message);
...
app.get('/api/partner/properties', authMiddleware, async (req, res) => {
  if (!dataEngine) return res.json({ success: true, count: 0, properties: [] });
  try {
...
app.post('/api/partner/properties', authMiddleware, async (req, res) => {
  if (!dataEngine) return res.status(503).json({ error: 'dataEngine não disponível' });
  try {
    const property = req.body;
    if (!property.id) property.id = `prop_${Date.now()}`;
    
    // Validar limite de fotos (máx. 10)
    const MAX_IMAGES = 10;
    const images = property.images || [];
    if (Array.isArray(images) && images.length > MAX_IMAGES) {
      return res.status(400).json({ 
        error: `Máximo de ${MAX_IMAGES} fotos por imóvel. Você enviou ${images.length}.` 
      });
    }
...
app.delete('/api/partner/properties', authMiddleware, async (req, res) => {
  if (!dataEngine) return res.status(503).json({ error: 'dataEngine não disponível' });
  try {
    const { id } = req.query;
...
app.get('/api/partner/property-image', authMiddleware, async (req, res) => {
  if (!dataEngine) return res.status(503).json({ error: 'dataEngine não disponível' });
  try {
    const { id } = req.query;
    if (!id) return res.status(400).json({ error: 'ID é obrigatório' });
...
app.get('/api/partner/properties/status', authMiddleware, async (req, res) => {
...
app.post('/api/marketing/criar-campanha', authMiddleware, async (req, res) => {
  try {
...
app.get('/api/marketing/status/:propertyId', authMiddleware, async (req, res) => {
  try {
...
app.get('/api/marketing/status', authMiddleware, (req, res) => {
  try {
...
app.get('/api/marketing/campanhas', authMiddleware, (req, res) => {
  try {
...
app.get('/api/campaigns/list', authMiddleware, async (req, res) => {
  try {
...
// Serve vídeo de um imóvel (para Instagram Reels)
